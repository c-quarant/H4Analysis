...
{	
	...
	Yshift_str = std::to_string(HodoPlaneShift(h4, detector, pathToLinFitOutput, RunStats, "Y"));
  	...
  	Nentries = h4->GetEntries();
   	...
   	h4->Draw(("amp_max["+detector+"]:Y[0]:X[0]>>AmpXY0").c_str(), "X[0]>-800 && Y[0]>-800");
  	h4->Draw(("amp_max["+detector+"]:Y[1]-("+Yshift_str+"):X[1]-("+Xshift_str+")>>AmpXY1").c_str(), "X[1]>-800 && Y[1]>-800");
  	h4->Draw(("amp_max["+detector+"]:(0.5*(Y[0]+Y[1]-("+Yshift_str+"))):(0.5*(X[0]+X[1]-("+Xshift_str+")))>>AmpXYM").c_str(), "X[0]>-800 && Y[0]>-800 && X[1]>-800 && Y[1]>-800");
