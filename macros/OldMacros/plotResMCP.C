...
void plotResMCP(std::string FileName, std::string detector, int NumberGains){
	gStyle->SetOptFit();	
	...
	if(NumberGains==1)
	{
		...
		g50->GetYaxis()->SetTitle("#sigma(MCP1-MCP2) (ps)");
	...
	if(NumberGains==2)
	{
		...
		g100->GetYaxis()->SetTitle("#sigma(MCP1-MCP2) (ps)");
	...
	if(NumberGains>=3)
	{
		...
		g200->GetYaxis()->SetTitle("#sigma(MCP1-MCP2) (ps)");
