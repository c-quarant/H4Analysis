...
WFClass::WFClass(int polarity, float tUnit):
    ...
    fWinMin_(-1), fWinMax_(-1), tempFitTime_(-1), tempFitAmp_(-1), fStatus_(-1), interpolator_(NULL)
{}
//**********Getters***********************************************************************
...
float WFClass::GetAmpMax(int min, int max)
{
    ...
    //---find the max
    maxSample_=sWinMin_;
    for(int iSample=sWinMin_; iSample<(int)sWinMax_*0.5; iSample++)
    {
...
WFFitResults WFClass::TemplateFit(bool NoiseCut, float offset, int lW, int hW)
{
	...
	*/
    ...
    return WFFitResults{tempFitAmp_, tempFitTime_, TemplateChi2()/(fWinMax_-fWinMin_-2), fStatus_};
}
