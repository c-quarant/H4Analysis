...
float AmplitudeProfilesFit(TTree* h4, std::string detector, std::string pathToOut, std::string RunStats, std::string axis, std:string Xshift, std:string Yshift,  std::string bound, std::string plane);
...
void AllAmplitudeProfilesFit(std::string FileIn, std::string detector, Float_t bound)
{
	...
	std::string pathToOutput = "/afs/cern.ch/user/c/cquarant/www/Amplitude_profilesNew/";
	...
	AmplitudeProfileFit(h4, detector, pathToOutput, RunStats, "X",  
...
float HodoPlaneShift(TTree* h4, std::string detector, std::string pathToOut, std::string RunStats, std::string axis)
{
	...
	h4->Draw(("("+axis+"[0]-"+axis+"[1]):"+axis+"[0]>>DXvsX").c_str(), (axis+"[0]>-800 && "+axis+"[1]>-800").c_str());
...
void AmplitudeProfilesFit(TTree* h4, std::string detector, std::string pathToOut, std::string Xshift, std::string Yshift, std::string bound, std::string plane, float* Xmax, float* Ymax)
{
	float fitrange=4;
	auto *AmpX = new TProfile(("AmpX"+plane).c_str(), "", 32, -16, 16, 0, 10000);

	varexp = "amp_max["+detector+"]:(X["+plane+"]+("+XShift+"))>>AmpX"+plane+"";
	selection = "Y["+plane+"]+("+YShift+ ")>-"+bound+" && Y["+plane+"]+("+YShift+")<"+bound;
	cout << varexp << endl;
	...
	TCanvas* c3 = new TCanvas("c3","c3");
 	TH2F* H1 = new TH2F("H1","", 32, -16, 16, 50, 0, (AmpX->GetMaximum())*1.25);
 	H1->GetXaxis()->SetTitle("X["+plane+"]");    
 	H1->GetYaxis()->SetTitle("amp_max");
  
	H1->Draw();
	AmpX->Fit("pol2", "Q", "", -fitRange, fitRange);		
	AmpX->Draw("SAME");
  
	fileOutpdf = pathToOut + "p"+plane+"_" + Energy + "Gev_" + "Amp"+axis+plane+"_" + std::to_string(runNum) + "_" + detector + "_G" + Gain + ".pdf";
  	fileOutpng = pathToOut + "p"+plane+"_" + Energy + "Gev_" + "Amp"+axis+plane+"_" + std::to_string(runNum) + "_" + detector + "_G" + Gain + ".png";
    
  	c3->SaveAs(fileOutpdf.c_str());
  	c3->SaveAs(fileOutpng.c_str());

	*Xmax = AmpX->GetFunction("pol2")->GetMaximumX();

	auto *AmpY = new TProfile(("AmpY"+plane).c_str(), "", 32, -16, 16, 0, 10000);

	varexp = "amp_max["+detector+"]:(Y["+plane+"]+("+YShift+"))>>AmpX"+plane+"";
	selection = "X["+plane+"]+("+XShift+ ")>-"+bound+" && X["+plane+"]+("+XShift+")<"+bound;
	cout << varexp << endl;
	cout << selectrion << endl;
	...
	TCanvas* c4 = new TCanvas("c4","c4");
 	H1->GetXaxis()->SetTitle("Y["+plane+"]");    
  
	H1->Draw();
	AmpY->Fit("pol2", "Q", "", -fitRange, fitRange);		
	AmpY->Draw("SAME");
