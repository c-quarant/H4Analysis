...
void FitTimeMean(std::string FileName, std::string detector, int NumberGains){
	gStyle->SetOptFit();	
	...
	*/
	if(NumberGains==1)
	{
		...
		g50->GetYaxis()->SetTitle("TimeMean(APD-MCP) (ps)");
	...
	if(NumberGains==2)
	{
		...
		g100->GetYaxis()->SetTitle("TimeMean(APD-MCP) (ps)");
	...
	if(NumberGains>=3)
	{
		...
		g200->GetYaxis()->SetTitle("TimeMean(APD-MCP) (ps)");
	...
	c0->SaveAs(("/afs/cern.ch/user/c/cquarant/www/fitTimeDist/TimeMean/TimeMean_vs_ANoise_"+detector+".png").c_str());
	c0->SaveAs(("/afs/cern.ch/user/c/cquarant/www/fitTimeDist/TimeMean/TimeMean_vs_ANoise_"+detector+".pdf").c_str());
